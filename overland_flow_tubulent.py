...
for i in range(len(time)-1):
    ...
    #hedge[-1] = h[-1]
    #U[:] = ((rho*g*np.sin(slope))/mu) * ((h[:,i]**2.)/3.) # calculate water velocity for this time and all space
#    U[:] = ((np.sqrt(g*h[:,i]*np.sin(slope))) / karman) * (np.log(z[:]/z0)-1)
